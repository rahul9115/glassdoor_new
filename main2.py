...
def scrape(field, review, author):
    ...
    def scrape_date(review):
        ...
        res = dt.datetime.strptime(jobtitle.split('-')[0].strip(' '),'%d %b %Y').date()
        return res
...
def collect_json():
    ...
    with open("network_log.json", "w", encoding="utf-8") as f:
        ...
        for log in logs:
            #print(log)
            #if "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9" in log:
            if json.loads(json.loads(json.loads(json.loads(json.loads(log["message"])['message'])['params'])['connectTiming'])['headers'])['Accept'] == "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9":
                network_log = json.loads(log["message"])["message"]

            #['message']['params']['connectTiming']['headers']['Accept']
...
def navigate_to_reviews():
    ...
    with open("page_src.html", "r", encoding="utf-8") as myfile:
        data = myfile.read()
        useful = data.split('"reviews":[')[1]
        useful = '{"reviews":[' + useful
        useful = useful.split('"translationMethod":null}]},"pageViewSummary"')[0]
        useful = useful + '"translationMethod":null}]}'

        with open("review_json.json", "w", encoding="utf-8") as f:
            f.write(useful)
        f.close()
    ...
    # useful = []
    # with open("page_src.html", "r") as myfile:
    #     for line in myfile:
    #         if '"reviews":[' in line:
    #             break
    #     for line in myfile:
    #         useful.append(line)
    #         if '"translationMethod":null}]},"pageViewSummary"' in line:
    #
...
def get_browser():
    ...
    browser = wd.Chrome(options=chrome_options, desired_capabilities=desired_capabilities)
    return browser
